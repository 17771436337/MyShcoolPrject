...
import com.example.a.myapplication.adapter.ScreenStyleAdapter;
...
public class ScreenStyleActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    ...
    ScreenStyleModel model = new ScreenStyleModel();

    ScreenStyleAdapter adapter;
    ...
    protected void initView() {
        initTitle();
        getData();
        adapter = new ScreenStyleAdapter(pullListView, model.getO());
        ...
        view.setTitleOnClickListeneRight(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (model != null && model.getO() != null) {
                    ScreenStyleModel towModel = new ScreenStyleModel();
                    ArrayList<ScreenStyleModel.Brand> list = new ArrayList<ScreenStyleModel.Brand>();
                    for (int i = 0; i < model.getO().size(); i++) {
                        ScreenStyleModel.Brand data = model.getO().get(i);
                        if (data.is()) {
                            list.add(data);
                        }
                    }
                    ...
                    intent.putExtra("style", towModel);
    ...
    private void getData() {
        ...
        if (!TextUtils.isEmpty(idname)) {
            par.put("idname", idname);
        }

        OkHttpUtil.getInstance().addRequestPost(Config.getIdol, par, new OkHttpUtil.HttpCallBack<ScreenStyleModel>() {

            @Override
            public void onSuccss(ScreenStyleModel screenBrandModel) {
    ...
    public void onEventMainThread(Object obj) {
        ...
        if (obj instanceof ScreenStyleModel) {
            ScreenStyleModel screenBrandModel = (ScreenStyleModel) obj;
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (model != null) {
            ScreenStyleModel.Brand data = model.getO().get(position - 1);
            if (data.is()) {
                data.setIs(false);
            } else {
                data.setIs(true);
